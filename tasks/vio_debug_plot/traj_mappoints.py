...
root_path = "/home/xuzhuo/Documents/data/01-mini/test/proj/R_RTK_TCI"
traj_file = f"{root_path}/gps_serial_node_2024_04_29_04_37_42.flf"
points_file = f"{root_path}/gps_serial_node_2024_04_29_04_37_42.flf.map"
ply_file = f"{root_path}/map.ply"

mps = np.loadtxt(points_file)
parser = ParseIPSSol()
header, sol = parser.parseSINS(traj_file)
x, y, z = sol["X-ECEF"], sol["Y-ECEF"], sol["Z-ECEF"]
test = np.array([x, y, z]).transpose()
...
mps_ply = [(mps_enu[i][0], mps_enu[i][1], mps_enu[i][2], int(mps[i, 3]), int(mps[i, 4]), int(mps[i, 5])) for i in range(1, len(mps_enu))]
coor_ply = [(coor_enu[i][0], coor_enu[i][1], coor_enu[i][2], int(0), int(255), int(0)) for i in range(1, len(coor_enu))]
